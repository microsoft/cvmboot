...
const char* TPM2X_CodeToStr(char buf[TPM2X_CODETOSTR_BUFSIZE], UINT32 rc)
{
    ...
    else if (rc & TPM_RC_N_MASK)
    {
        ...
        if (str)
        {
            ...
            snprintf(buf, TPM2X_CODETOSTR_BUFSIZE, "%s(%u)", str, n);
        }
    }
    ...
    TPMI_SH_AUTH_SESSION *policySession)
{
    ...
    if (alg == TPM_ALG_SHA1)
    {
        nonceCaller.size = sizeof(sha1_t);
    }
    else if (alg == TPM_ALG_SHA256)
    {
        nonceCaller.size = sizeof(sha256_t);
    }
    else
    {
        /* Unknown hash. */
    ...
    err_t* err)
{
    ...
    {
        err_format(err, "Unsupported hash function");
        return TPM_RC_FAILURE;
    }
    ...
    if ((rc = TPM2_PolicyPassword(
        ...
        NULL)) != TPM_RC_SUCCESS)
    {
        err_format(err, "failed to set the policy password");
        return rc;
    }
    ...
    else
    {
        if ((rc = TPM2_PCR_Read(
            ...
            NULL)) != TPM_RC_SUCCESS)
        {
            err_format(err, "failed to read PCRs when building policy");
            return rc;
        }
    }
    ...
    {
        err_format(err, "unexpected error when building policy");
        return TPM_RC_FAILURE;
    }
    ...
    TPM2B_DIGEST pcrDigest;
    {
        ...
        {
            sha1_compute((sha1_t*)pcrDigest.buffer, data, end - data);
            pcrDigest.size = sizeof(sha1_t);
        }
        else /* SHA256 */
        {
            sha256_compute((sha256_t*)pcrDigest.buffer, data, end - data);
            pcrDigest.size = sizeof(sha256_t);
        }
    }
    ...
    if ((rc = TPM2_PolicyPCR(
        ...
        NULL)) != TPM_RC_SUCCESS)
    {
        err_format(err, "failed to create PCR policy");
        return rc;
    }
    ...
    err_t* err)
{
    ...
    if (!protocol)
    {
        err_format(err, "null protocol parameter");
        rc = TPM_RC_FAILURE;
        goto done;
    }
    ...
    if (!data)
    {
        err_format(err, "null data parameter");
        rc = TPM_RC_FAILURE;
        goto done;
    }
    ...
    {
        err_format(err, "size parameter too big");
        rc = TPM_RC_FAILURE;
        goto done;
    }
    ...
    if (!blob)
    {
        err_format(err, "null blob parameter");
        rc = TPM_RC_FAILURE;
        goto done;
    }
    ...
    {
        err_format(err, "failed to start authentication session");
        return rc;
        goto done;
    }
    ...
    if ((rc = TPM2_PolicyGetDigest(
        ...
        NULL)) != TPM_RC_SUCCESS)
    {
        err_format(err, "failed to get the policy digest");
        goto done;
    }
    ...
    /* Create */
    {
        ...
        {
            err_format(err, "failed to create sealed object");
            goto done;
        }
    ...
    err_t* err)
{
    ...
    if (hashAlg != TPM_ALG_SHA1 && hashAlg != TPM_ALG_SHA256)
    {
        err_format(err, "unsupported hashing algorithm");
        rc = TPM_RC_FAILURE;
        goto done;
    }
...

    {
        ...
        {
            err_format(err, "failed to load sealed blob");
            goto done;
        }
    ...
    /* Unseal the sealed blob */
    {
        ...
        if (rc != TPM_RC_SUCCESS)
        {
            err_format(err, "TPM_Unseal Failed %u", rc);
            goto done;
        }
    ...
    IN TPMI_DH_PCR pcrHandle)
{
    ...
    /* SHA1 Capping */
    {
        ...
        sha1_compute(&sha1, zeros, sizeof(zeros));
    ...
    /* SHA256 Capping */
    {
        ...
        sha256_compute(&sha256, zeros, sizeof(zeros));
        memset(&digests, 0, sizeof(digests));
        digests.count = 1;
    ...
    EFI_TCG2_PROTOCOL* protocol)
{
    ...
    for (i = 0; i < PCR_COUNT; i++)
    {
        ...
        if (sha256_equal(&sha256, &sha256s[i]))
        {
            rc = TPM_RC_FAILURE;
            goto done;
        }
    ...
    sha1_t* sha1)
{
    ...
    if (pcrValues.digests[0].size != sizeof(sha1_t))
    {
        return TPM_RC_FAILURE;
    }

    memcpy(sha1, pcrValues.digests[0].buffer, sizeof(sha1_t));
    return TPM_RC_SUCCESS;
}
    ...
    sha256_t* sha256)
{
    ...
    if (pcrValues.digests[0].size != sizeof(sha256_t))
    {
        return TPM_RC_FAILURE;
    }

    memcpy(sha256, pcrValues.digests[0].buffer, sizeof(sha256_t));
    return TPM_RC_SUCCESS;
}
    ...
    IN const sha1_t* sha1In)
{
    ...
    digests.count = 1;
    digests.digests[0].hashAlg = TPM_ALG_SHA1;
    memcpy(digests.digests[0].digest.sha1, (BYTE*)sha1In, sizeof(sha1_t));
    ...
    IN const sha256_t* sha256In)
{
    ...
    digests.count = 1;
    digests.digests[0].hashAlg = TPM_ALG_SHA256;
    memcpy(digests.digests[0].digest.sha256, sha256In,sizeof(sha256_t));
    ...
    IN UINT16 size)
{
    ...
    /* Dump TPM2B_PRIVATE */
    {
        ...
        printf("TPM2B_PRIVATE.size: ");
        printf("%lld\n", (long long)blob.private.size);
        printf("TPM2B_PRIVATE.buffer:\n");
        hexstr_dump(blob.private.buffer, blob.private.size);
    }
    ...
    /* Dump TPM2B_PUBLIC */
    {
        ...
        printf("TPM2B_PUBLIC.size: ");
        printf("%lld\n", (long long)blob.public.size);
        printf("TPM2B_PUBLIC.publicArea:\n");
        hexstr_dump(&blob.public.publicArea, blob.public.size);
    }
    ...
    /* Dump TPM2B_DIGEST */
    {
        ...
        printf("TPM2B_DIGEST.size: ");
        printf("%lld\n", (long long)blob.hash.size);
        printf("TPM2B_DIGEST.buffer:\n");
        hexstr_dump(blob.hash.buffer, blob.hash.size);
    }
    ...
    /* Dump TPMS_PCR_SELECT */
    {
        ...
        printf("TPMS_PCR_SELECT.size: ");
        printf("%lld\n", (long long)blob.pcr.sizeofSelect);
        printf("TPMS_PCR_SELECT.buffer:\n");
        hexstr_dump(blob.pcr.pcrSelect, blob.pcr.sizeofSelect);
    }
    ...
    UINTN count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        sha256_format(&sha256Str, &sha256);
        printf("PCR[%02d]=%s\n", i, sha256Str.buf);
    }
